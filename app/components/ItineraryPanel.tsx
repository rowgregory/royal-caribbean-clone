...
const ItineraryPanel = ({
  ...
  const cruiseDetails = cruise?.availableDates?.map((dates: any) => {
      ...
      >
        ...
        <section className="px-5 mt-4 w-full">
          ...
          <div className="flex justify-between w-full mt-4">
            {cruise?.availableDates &&
              cruise?.availableDates[0]?.rooms?.map((room: any, i: number) => (
                <div
                  key={i}
                  className={`${
        ...
        <div>
          {cruiseDetails?.map((details: any, i: number) => {
            return (
              ...
              >
                <div
                  className={`grid grid-cols-4 items-center justify-between px-5 py-4 transition duration-300 hover:bg-yellow-400 ${
                    details.dateId === id ? 'bg-yellow-400' : ''
                  }`}
                >
